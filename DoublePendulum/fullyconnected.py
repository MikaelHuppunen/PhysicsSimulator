...
class DoublePendulum:
    ...
    def get_next_state(self, state, action):
        ...
        if state[2] > self.max_angular_speed:
            state[2] = self.max_angular_speed
        if state[2] < -self.max_angular_speed:
            state[2] = -self.max_angular_speed
        if state[3] > self.max_angular_speed:
            state[3] = self.max_angular_speed
        if state[3] < -self.max_angular_speed:
            state[3] = -self.max_angular_speed
    ...
    def simulate_action(self, state):
        theta_1, theta_2, dot_theta_1, dot_theta_2 = state[0], state[1], state[2], state[3]
        for i in range(100):
...
def learn(args, system):
    model = ResNet(system, 8, 64, device=device, number_of_input_channels=1)
