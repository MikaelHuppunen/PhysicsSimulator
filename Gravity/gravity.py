...
def gravity(index, gravitational_constant, mass, position, radius):
    acceleration = [0,0,0]
...
def approximate(delta_t, mass, velocity, position, radius, gravitational_constant):
    for i in range(len(mass)):
        acceleration = gravity(i, gravitational_constant, mass, position, radius)
        for axel in range(3):
...
def simulate(duration, delta_t):
    ...
    while t < duration:
        for i in range(simulation_interval):
            approximate(delta_t, mass, velocity, position, radius, gravitational_constant)
            t += delta_t
...
if __name__ == "__main__":
    ...
    while running:
        ...
        if mode == 0:
            for i in range(simulation_interval):
                approximate(dt, mass, velocity, position, radius, gravitational_constant)
                t += dt
            for i in range(len(mass)):
